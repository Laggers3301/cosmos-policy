...
def main():
    ...
    for i, parquet_path in enumerate(parquet_paths):
        try:
            ...
            # Wrap wrist joints (dim5=left_joint[5], dim12=right_joint[5]) to [-pi, pi]
            # to fix cross-session zero-point inconsistency (~27% of episodes affected)
            qpos[:, 5] = ((qpos[:, 5] + np.pi) % (2 * np.pi)) - np.pi
            qpos[:, 12] = ((qpos[:, 12] + np.pi) % (2 * np.pi)) - np.pi
